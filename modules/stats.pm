...
sub creatingStatFileTex
{
	...
	{
          toolbox::exportLog("----- creatingStatFileTex : Je lis 1 $file", 1);    
	...
	\\begin{table}[ht]
		...
		\\begin{tabular}{l|r|r|r|r|r|r|r|r|r|r|r}
			Samples & Raw sequences & Lenght & Length mean & Length min & qual min & qual max & qual mean & \\%A & \\%C & \\%G & \\%T    \\\\\\hline \n" if  ($texFastq eq 'NA');
			...
			while (my $line = <$fh>)
			{
				...
				elsif ($line =~ /\slen\smean\(/) { $lenMean = $val; }
                    elsif ($line =~ /\slen\smin\(/) { $lenMin = $val; }
                    elsif ($line =~ /\squal\smin\(/) { $qualMin = $val; }
                    elsif ($line =~ /\squal\smax\(/) { $qualMax = $val; }
                    elsif ($line =~ /\squal\smean\(/) { $qualMean = $val; }
                    elsif ($line =~ /%A/) { $A = $val; }
                    elsif ($line =~ /%C/) { $C = $val; }
                    elsif ($line =~ /%G/) { $G = $val; }
                    elsif ($line =~ /%T/) { $T = $val; }
			...
			$texMapping .= " $sample & $read & $len & $lenMean & $lenMin & $qualMin & $qualMax & $qualMean & $A & $C  & $G & $T & $N & $totBase \\\\ \n";
